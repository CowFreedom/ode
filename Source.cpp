...
void testlotkavolterra() {
	...
	std::array<double, 2> h_values = { 0.00001,5 };
//	auto result = r::ode45(functions, tspan, u0, h_values);
	std::cout << "DONE!\n";
...
void testalgorithms() {
	...
	std::array<double, 2> tspan = { t_start,10 };
	...
	//auto result = r::ode2(functions, tspan, u0, h);
	std::array<double, 2> h_values = { 0.01,5 };
	auto result = r::ode45(functions, tspan, u0, h_values);
	std::cout << "t:\tu:\n";
	for (size_t i = 0; i < (result.second).size(); i++) {
		auto t = (result.first)[i];
...
void test_adaptive() {
	...
	std::array<double, 2> tspan = { -5,5 };
	std::array<double, 2> u0 = { 3.9968E-4 };
	auto u1 = [&](std::vector<double>&& u, double t) {
		return -200 * t*pow(u[0],2);
	};
	std::vector<std::function<double(std::vector<double>, double)>> functions;
	functions.push_back(u1);

	std::array<double, 2> h_values = { h_min,h_max };
	auto result = r::ode45(functions, tspan, u0, h_values);
//	printTrajectory(result);

	//Tatschliche Funktion
	auto r1 = [&](double t) {
		return (0.01)/(pow(t,2)+0.020016);
	};
	std::vector<std::function<double(double)>> real_functions;
	real_functions.push_back(r1);
	std::cout << "Globaler Fehler (u-y)/h:\n";
	...
	for (size_t i = 0; i < (result.second).size(); i++) {
		auto t = (result.first)[i];
		std::cout << t << "\t";
		std::cout << (result.second)[i][0] - (real_functions[0](t)) << "\t";
		std::cout << "\n";
	}
...
void sample_ode() {
	...
	functions.push_back(u1);
	functions.push_back(u2);
	std::array<double, 2> h_values = { 0.02,1 };
	auto result = r::ode45(functions, tspan, u0, h_values);
