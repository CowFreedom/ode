...
namespace r {
	...
	auto Explicit_variable_RKTemplate(RKTableInterface<alg, rk_Container1D, rk_Container2D>& butcher_tableau, const fContainer& u, const numberContainer& tspan, const initContainer& u0, const numberContainer& h_values, measure rule);
...
//	auto ode1(const std::initializer_list<fContainer>& u, const std::initializer_list<numberContainer>& tspan, const std::initializer_list<initContainer>& u0, T h);
	...
	auto ode45(const fContainer& u, const numberContainer& tspan, const initContainer& u0, const numberContainer& h_values);
}
